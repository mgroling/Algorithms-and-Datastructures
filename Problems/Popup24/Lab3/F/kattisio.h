...
std::string format_output(const std::vector<std::tuple<bool, long long, long long>> &solution)
{
    std::string output = "";

    for (const std::tuple<bool, long long, long long> &answer : solution)
    {
        if (std::get<0>(answer))
        {
            output += std::to_string(std::get<1>(answer)) + ' ' + std::to_string(std::get<2>(answer)) + '\n';
        }
        else
        {
            output += "no solution\n";

  ...
  public:
    ...
    int inside_polygon(const std::vector<Point<T>> &polygon)
    {
        assert(polygon[0] == polygon.back() &&
               "Assert failed in Point.inside_polygon: first point of polygon must equal last");
        double angle = 0;
        T dot, cross;

        for (int i = 0; i < polygon.size() - 1; i++)
        {
            dot = (polygon[i] - *this).dot_product(polygon[i + 1] - *this);
            cross = (polygon[i] - *this).cross_product(polygon[i + 1] - *this);

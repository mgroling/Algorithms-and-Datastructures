...
struct Test_case
{
    ...
    Test_case(int num_nodes_, std::vector<std::pair<int, int>> edges_)
    {
...
std::vector<Test_case> parse_input(std::vector<std::string> input)
{
    ...
    for (int i = 0; i < input.size();)
    {
        ...
        for (int j = 0; j < num_edges; j++)
        {
            ...
            edges.emplace_back(std::stoi(split_string[0]), std::stoi(split_string[1]));
        }
        i += num_edges;

        output.emplace_back(num_nodes, edges);
    }
    return output;
}
